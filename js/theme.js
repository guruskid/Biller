...
----------------------------------------------------- */
  ...
  $(".login-signup ul.navbar-nav > li.dropdown > .dropdown-menu").each(
    function () {
      var menu = $("#header .header-row").offset();
      var dropdown = $(this).parent().offset();
      if ($("html").attr("dir") == "rtl") {
        var rd =
          $(window).width() - (dropdown.left + $(this).parent().outerWidth());
        var i =
          rd +
          $(this).outerWidth() -
          (menu.left + $("#header .header-row").outerWidth());
...
function setupNavigation() {
  ...
  let asideOffSetTop = asideBar.offsetTop;
  ...
  // Slides the navigation in and out on the click of the menu
  navBarToggler.addEventListener("click", () => {
    if (navBarToggler.classList.contains("open")) {
      asideBar.style.transform = "translateX(0)";
    } else {
      asideBar.style.transform = "translateX(-250px)";
    }
  });

  // Closes the navigation when the screen outside the navigation is clicked
...
// Run the setupNavigation function when the page has finished loading
